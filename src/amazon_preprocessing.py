...
def __main_amazon_preprocessing__(strata_portion=.001):
    name ="amazon"

    
    phoneme_data_size= 13647978
    strata_size = int(strata_portion*phoneme_data_size)
    path = os.getcwd()+"/src/Data/Amazon/amazon_review.csv"
    dims = ["overall","date","vote","age","category","country"]
    aggregation_dims =  ["overall","vote","age","category","country"]#,"date"
    #,"date"
    domain =[5,100000,10,70,28,196]
    reduced_domain= []
