...
# To be completed
def __to_numerical_dims__() ->None:
    cnx = Connection()
...
def __clean_db__(table_name:str):
    try:
        cnx = Connection()
        cursor = cnx.conn.cursor()
        ...
        for strata_num in range(number_stratas):
            query = sql_data.__delete_cluster__(table_name,strata_num)
            cursor.execute(query)
            #with meta
            query  = sql_data.__delete_cluster_meta__(table_name,strata_num)
            cursor.execute(query)
            cnx.conn.commit()
...
def __remove_stratas__(table_name:str,strata_size:int):
    cnx = Connection()
    cursor = cnx.conn.cursor()
...
def __insert_synth_data__(table_name:str,dims_declarations:str,dims_agg_s:str,domain,number_rows:int):
    cnx = Connection()
    cursor = cnx.conn.cursor()
...
def __load_data_file__(table_name:str,dims_declarations:str,path:str,separator:str,header:str):
    cnx = Connection()
    cursor = cnx.conn.cursor()
...
def __aggregate_table_to_temp__(table_name:str,dims_agg_declarations:str,dims_agg:str):
    cnx = Connection()
    cursor = cnx.conn.cursor()
...
def __create_index__(table_name:str,agg_dims_declaration_min:str,agg_dims_declaration_max:str):
    cnx = Connection()
    cursor = cnx.conn.cursor()
...
def __create_clusters__(table_name:str,strata_size:int,agg_dims_declaration_s:str,dim_agg:str,dims_agg_s:str,domain,with_meta=True,with_sort=False)->int:
    cnx = Connection()
    cursor = cnx.conn.cursor()
    query = sql_data.__get_size_temp__(table_name)
    cursor.execute(query)
    ...
    for strata_num in tqdm(np.arange(number_stratas), " Stratas loop : ",leave=True):
        ...
        query = sql_data.__create_cluster__(table_name,strata_num,agg_dims_declaration_s)
        cursor.execute(query)
            
        query = sql_data.__populate_cluster__(table_name,strata_num,strata_size,dim_agg,dims_agg_s)
        cursor.execute(query)
        cnx.conn.commit()
        
        # meta works with Domain should be changed to get distinct to reduce the search space
        __metadata_cluster__(table_name,strata_size,dims_agg_s,strata_num,domain)

        __cluster_BE_index__(table_name,strata_size,dims_agg_s,strata_num,domain)
...
def __add_dbms_indexes_to_tables__(table_name:str,agg_dims:str,dims_min:str,dims_max:str,dims_agg:str):
    cnx = Connection()
    cursor = cnx.conn.cursor()
    # table to test
...
def __metadata_cluster__(table_name:str,strata_size:int,dims_agg_s,strata_num:int,domain):
    cnx = Connection()
    cursor = cnx.conn.cursor()

    cursor.execute(sql_index.__create_meta_cluster__(table_name,strata_num))
    ...
    for index_dim,dim in enumerate(dims_agg_s):
        ...
        for member in range(-1,domain[index_dim]+1):
            ...
            cursor.execute(sql_index.__add_to_cluster_meta__(table_name,strata_num,dim,str(member),p_l,p_g))

    cnx.conn.commit()


def __cluster_BE_index__(table_name:str,strata_size:int,dims_agg_s:str,strata_num:int,domain):
        
        cnx = Connection()
        cursor = cnx.conn.cursor()
        query = sql_data.__avg_cluster__(table_name,strata_num)
        cursor.execute(query)
        x =cursor.fetchone()[0]
        avg_strata = str(x)
        # to compute the info for the index we need : min,max, spar = size/ volume(max-min), avg
        query = sql_data.__min_row_cluster_sorted_by_dims__(table_name,strata_num,dims_agg_s)
        cursor.execute(query)
        min_dim = ""
        min_dim_v=[]
        ...
        query = sql_data.__max_row_cluster_sorted_by_dims__(table_name,strata_num,dims_agg_s)
        cursor.execute(query)
        ...
        query = sql_index.__add_cluster_to_index__(table_name,strata_num,min_dim,max_dim,avg_strata,str(sparsity))
        cursor.execute(query)
        cnx.conn.commit()