...
def __aggregate_and_sort_by_count_in_temp__(name:str,dims_names:str)->str:
    return """insert into """+name+"""_temp ("""+dims_names+""",agg_count)
        ...
        order by (agg_count);
        """
def __create_cluster__(name:str,num:int,dims_declarations:str)->str:
    return "CREATE TABLE "+name+"_" +str(num)+" (id_row serial ,"+dims_declarations+", agg_count int);"
def __create_cluster_temp__(name:str,num:int,dims_declarations:str)->str:
    return "CREATE TABLE "+name+"_temp_" +str(num)+" (id_row serial ,"+dims_declarations+", agg_count int);"
def __drop_cluster_temp__(name:str,num:int)->str:
    return "drop TABLE "+name+"_temp_" +str(num)+" "

def __delete_cluster__(dataset:str,num:int)->str:
    return "drop TABLE "+dataset+"_" +str(num)+";"
def __delete_cluster_meta__(dataset:str,num:int)->str:
    return "drop TABLE "+dataset+"_" +str(num)+"_meta;"
def __delete_cluster_temp__(dataset:str,num:int)->str:
    return "drop TABLE "+dataset+"_temp_" +str(num)+";"

def __populate_cluster__(name:str,num:int,size:int,dims_agg:str,dims_agg_s:str)->str:
    return "insert into "+name+"_"+str(num)+"""  ("""+dims_agg_s+""",agg_count)
        ...
        """
def __populate_cluster_sort__(name:str,num:int,size:int,dims_agg:str,dims_agg_s:str)->str:
    return "insert into "+name+"_"+str(num)+"""  ("""+dims_agg_s+""",agg_count)
        (select """+dims_agg_s+""", agg_count From """+name+"_temp_"+str(num)+""" order by """+dims_agg_s+"""
        );
        """
def __populate_temp_cluster__(name:str,num:int,size:int,dims_agg:str,dims_agg_s:str)->str:
    return "insert into "+name+"_temp_"+str(num)+"""  ("""+dims_agg_s+""",agg_count)
        (select """+dims_agg+""" From """+name+"""_temp where id_row between """+ str(num*size)+ """ and """+ str(num*size+size)+ """);
        """
def __avg_cluster__(name:str,num:int)->str:
    return "select AVG(agg_count) from "+name+"_"+str(num)+";"
def __min_row_cluster_sorted_by_dims__(name:str,num:int,dims_agg_s:str) ->str:
    return "select "+dims_agg_s+" from "+name+"_"+str(num)+" where id_row = 1 ;"
def __max_row_cluster_sorted_by_dims__(name:str,num:int,dims_agg_s:str) ->str:
...
def __execute_query_cluster__(name:str,operator:str,num:int,range_sql:str):
